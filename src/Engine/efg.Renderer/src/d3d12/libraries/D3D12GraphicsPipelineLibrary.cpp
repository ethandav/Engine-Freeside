...
namespace efg::d3d12
{
    void D3D12GraphicsPipelineLibrary::Initialize(D3D12Context* device, const D3D12ShaderLibrary& shaderLibrary)
    {
        m_graphicsContext = device;
        CreateForwarLitGeometryPipeline(shaderLibrary);
        CreateShadowMapPipeline(shaderLibrary);
        CreateSkyboxPipeline(shaderLibrary);
    }

    const GraphicsPipelineState& D3D12GraphicsPipelineLibrary::Get(PipelineId id) const
    {
    ...
    void D3D12GraphicsPipelineLibrary::CreateForwardLitRootSignature(ID3D12RootSignature** rootSignature)
    {
    ...
    void D3D12GraphicsPipelineLibrary::CreateShadowMapRootSignature(ID3D12RootSignature** rootSignature)
    {
        CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc;
    ...
    void D3D12GraphicsPipelineLibrary::CreateSkyboxRootSignature(ID3D12RootSignature** rootSignature)
    {
        CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc;
    ...
    void D3D12GraphicsPipelineLibrary::CreateForwarLitGeometryPipeline(const D3D12ShaderLibrary& shaderLibrary)
    {
    ...
    void D3D12GraphicsPipelineLibrary::CreateShadowMapPipeline(const D3D12ShaderLibrary& shaderLibrary)
    {
        GraphicsPipelineState pipeline = {};
    ...
    void D3D12GraphicsPipelineLibrary::CreateSkyboxPipeline(const D3D12ShaderLibrary& shaderLibrary)
    {
        GraphicsPipelineState pipeline = {};
        CreateSkyboxRootSignature(pipeline.rootSignature.GetAddressOf());
        D3D12_INPUT_ELEMENT_DESC inputElementDescs[] =
        {
            { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }
        };
        ...
        AddGraphicsPipeline(PipelineId::Skybox, pipeline);
    }
}