...
namespace Freeside::Editor
{
	...
	void EditorInterface::DrawInspector(Scene::Scene& scene)
	{
		...
		{
			DrawMeshRendererComponent(scene.GetMeshRenderer(entity));
		}
	...
	void EditorInterface::DrawMeshRendererComponent(MeshRendererComponent* meshRenderer)
	{
		if (ImGui::CollapsingHeader("Mesh Renderer", ImGuiTreeNodeFlags_DefaultOpen))
		{
			ImGui::Text("Mesh handle: %u", meshRenderer->mesh.index);
			ImGui::Text("Material handle: %u", meshRenderer->material.index);

			// Later:
			// drag/drop asset assignment
			// combo box of loaded meshes/materials
		}
	}
